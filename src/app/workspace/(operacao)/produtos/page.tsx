      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[720px] text-left text-sm">
              ...
              <tbody className="divide-y divide-neutral-100">
                {workspace.stockItems.map((item) => <tr key={item.id}><td className="px-4 py-3 font-medium">{item.name}</td><td className="px-4 py-3 text-neutral-600">{item.categoryId ? categoryNames.get(item.categoryId) ?? "—" : "—"}</td><td className="px-4 py-3 text-neutral-600">{item.baseUnitCode}</td><td className="px-4 py-3 text-neutral-600">{itemTypeLabels[item.type]}</td><td className="px-4 py-3 text-neutral-600">{item.trackExpiration ? "Controlada" : "Não"}</td><td className="px-4 py-3"><span className={`rounded-full px-2 py-1 text-xs font-medium ${item.active ? "bg-emerald-50 text-emerald-700" : "bg-neutral-100 text-neutral-500"}`}>{item.active ? "Ativo" : "Inativo"}</span></td><td className="px-4 py-3 text-right">{workspace.permissions.manageCatalog && <button type="button" onClick={() => startEdit(item)} className="rounded-lg border border-neutral-200 px-3 py-1.5 font-medium hover:bg-neutral-50">Editar</button>}</td></tr>)}
        ...
        {workspace.permissions.manageCatalog ? (
          <form onSubmit={submit} className="h-fit space-y-4 rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm">
            ...
            <label className="block text-sm font-medium">Categoria<select value={form.categoryId} onChange={(event) => setForm({ ...form, categoryId: event.target.value })} className="mt-1 w-full rounded-lg border border-neutral-300 px-3 py-2 font-normal"><option value="">Sem categoria</option>{workspace.categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></label>
