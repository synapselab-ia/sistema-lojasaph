...
export default async function OrganizationSelectionPage({ searchParams }: OrganizationPageProps) {
  ...
  return (
    <main className="mx-auto min-h-screen max-w-3xl px-5 py-10 sm:py-16">
      <header>
        <p className="text-sm font-semibold uppercase tracking-[0.16em] text-neutral-500">Escopo de acesso</p>
        <h1 className="mt-2 text-3xl font-semibold tracking-tight">Selecione a organização</h1>
        <p className="mt-2 max-w-2xl text-sm leading-6 text-neutral-600">A seleção fica em cookie httpOnly, mas é revalidada contra seus memberships a cada carregamento protegido.</p>
      </header>
      ...
      <div className="mt-7 grid gap-4 sm:grid-cols-2">
        {context.organizations.map((organization) => (
          <form key={organization.id} action={selectOrganizationAction} className="rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm">
            ...
            <p className="mt-1 text-xs text-neutral-500">Perfis: {organization.roles.join(", ")}</p>
